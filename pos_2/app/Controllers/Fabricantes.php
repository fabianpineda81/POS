...
class Fabricantes extends BaseController{
    ...
    public function index($activo=1){
        ...
        // asi se hace una colsulta simplemente se pone la restricion, para ver mas variables de 'findAll()' ver la documentacion 
            $fabricantes = $this->fabricantes->where('activo',$activo)->findAll();  
            // array de datos que se le envia a la vista 
            $data =['titulo'=>'Estantes','datos'=>$fabricantes];
    ...
    public function eliminados($activo=0){
        ...
        // asi se hace una colsulta simplemente se pone la restricion, para ver mas variables de 'findAll()' ver la documentacion 
            $unidades = $this->fabricantes->where('activo',$activo)->findAll();  
            // array de datos que se le envia a la vista 
    ...
    public function id($id){
        ...
        $this->fabricantes->select('*');
        $this->fabricantes->where('id', $id);
        $this->fabricantes->where('activo', 1);
        $datos = $this->fabricantes->get()->getrow();
        $res["datos"]=false;
        $res["datos"]="";
        $res["error"]="";
        if($datos){
